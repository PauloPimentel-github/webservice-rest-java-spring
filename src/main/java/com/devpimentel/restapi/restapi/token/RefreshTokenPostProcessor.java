package com.devpimentel.restapi.restapi.token;
...
public class RefreshTokenPostProcessor implements ResponseBodyAdvice<OAuth2AccessToken> {
    ...
    public OAuth2AccessToken beforeBodyWrite(OAuth2AccessToken body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        ...
        String refreshToken = body.getRefreshToken().getValue(); // recupera o valor do token
        addRefreshTokenCookie(refreshToken, req, resp);
        deleteTokenBody(token);

        return body;
    }

    private void deleteTokenBody(DefaultOAuth2AccessToken token) {
    ...
    private void addRefreshTokenCookie(String refreshToken, HttpServletRequest req, HttpServletResponse resp) {
        ...
        refreshTokenCookie.setSecure(restApiProperty.getSecurity().isEnableHttps());
